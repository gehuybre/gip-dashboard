...
class GIPMap {
    ...
    async loadData() {
        try {
            ...
            const url = `./small_projects.json?v=${timestamp}&r=${random}`;
            ...
            if (!text.trim().startsWith('{')) {
                throw new Error('Response does not appear to be JSON: ' + text.substring(0, 200));
            }
            ...
            try {
                ...
                for (let i = 0; i < Math.min(text.length, 1000); i++) {
                    ...
                    if (charCode < 32 && charCode !== 9 && charCode !== 10 && charCode !== 13) {
                        problematicChars.push({ pos: i, char: text.charAt(i), code: charCode });
                    }
                }
                
                if (problematicChars.length > 0) {
                    console.error('Found problematic characters:', problematicChars.slice(0, 5));
                }
                
                throw new Error(`JSON parsing failed: ${parseError.message}. Response length: ${text.length}`);
            }
